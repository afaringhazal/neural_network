...
class FC(Abs_layer):
    ...
    def forward(self, A_prev):
        ...
        if len(A_prev_tmp.shape) == 4: # check if A_prev is output of convolutional layer
            # print("-----------------------------------")
            count_of_features = A_prev_tmp.shape[1] * A_prev_tmp.shape[2] * A_prev_tmp.shape[3]
            A_prev_tmp = A_prev_tmp.reshape(count_of_features, -1).T
        self.reshaped_shape = A_prev_tmp.shape
        
        # TODO: Forward part
        W, b = self.parameters
        # print(f'A_prev shape {A_prev_tmp.shape}')
    ...
    def backward(self, dZ, A_prev):
        ...
        # TODO: backward part
        W, b = self.parameters
        dW = A_prev_tmp.T @ dZ / A_prev_tmp.shape[0]
